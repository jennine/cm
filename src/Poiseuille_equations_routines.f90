...
999 CALL ERRORS("POISEUILLE_EQUATION_EQUATIONS_SET_SUBTYPE_SET",ERR,ERROR)
  ...
  SUBROUTINE POISEUILLE_EQUATION_EQUATIONS_SET_STATIC_SETUP(EQUATIONS_SET,EQUATIONS_SET_SETUP,ERR,ERROR,*)
    ...
    TYPE(FIELD_TYPE), POINTER :: INDEPENDENT_FIELD,MATERIAL_FIELD
    TYPE(VARYING_STRING) :: LOCAL_ERROR
    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      IF(EQUATIONS_SET%SUBTYPE==EQUATIONS_SET_STATIC_POISEUILLE_SUBTYPE) THEN
        SELECT CASE(EQUATIONS_SET_SETUP%SETUP_TYPE)
          CASE(EQUATIONS_SET_SETUP_INITIAL_TYPE)!this tells it that it is the FEM method
...
!!TODO: Check valid set up ?is this only for initial ??
          CASE DEFAULT
            ...
            IF(ASSOCIATED(EQUATIONS_MATERIALS)) THEN
              IF(EQUATIONS_MATERIALS%MATERIALS_FIELD_AUTO_CREATED) THEN
                ...
                CALL FIELD_DATA_TYPE_SET_AND_LOCK(EQUATIONS_MATERIALS%MATERIALS_FIELD,FIELD_U_VARIABLE_TYPE, &
                  & FIELD_DP_TYPE,ERR,ERROR,*999)
                !Set the number of materials components (2)
                CALL FIELD_NUMBER_OF_COMPONENTS_SET_AND_LOCK(EQUATIONS_MATERIALS%MATERIALS_FIELD,FIELD_U_VARIABLE_TYPE, &
                  & 2,ERR,ERROR,*999)
                !Default the 2 materials components to the first geometric interpolation setup with constant interpolation
	...
	CASE(EQUATIONS_SET_SETUP_INDEPENDENT_TYPE)
	     ...
	     EQUATIONS_INDEPENDENT=>EQUATIONS_SET%INDEPENDENT
 		   ...
 		   !note only set up for user defined fields
                    CALL FIELD_TYPE_CHECK(EQUATIONS_SET_SETUP%FIELD,FIELD_GENERAL_TYPE,ERR,ERROR,*999)
                    CALL FIELD_DEPENDENT_TYPE_CHECK(EQUATIONS_SET_SETUP%FIELD,FIELD_INDEPENDENT_TYPE,ERR,ERROR,*999)
                    CALL FIELD_NUMBER_OF_VARIABLES_CHECK(EQUATIONS_SET_SETUP%FIELD,1,ERR,ERROR,*999)
                    CALL FIELD_VARIABLE_TYPES_CHECK(EQUATIONS_SET_SETUP%FIELD,[10],ERR,ERROR,*999)!10=Radius Field Type
                    CALL FIELD_DIMENSION_CHECK(EQUATIONS_SET_SETUP%FIELD,10,FIELD_VECTOR_DIMENSION_TYPE, & 
                  & ERR,ERROR,*999)
                    CALL FIELD_DATA_TYPE_CHECK(EQUATIONS_SET_SETUP%FIELD,10,FIELD_DP_TYPE,ERR,ERROR,*999)
                    CALL FIELD_NUMBER_OF_COMPONENTS_CHECK(EQUATIONS_SET_SETUP%FIELD,10,1,ERR,ERROR,*999)
                 ELSE
		...
		EQUATIONS_INDEPENDENT=>EQUATIONS_SET%INDEPENDENT
            	IF(ASSOCIATED(EQUATIONS_INDEPENDENT)) THEN
		  !do nothing for a user generated field
            	ELSE
              	CALL FLAG_ERROR("Equations set independent is not associated.",ERR,ERROR,*999)
           	 ENDIF
          CASE DEFAULT
